...
export class WorkflowStore {
  ...
  private async save(): Promise<void> { if (!this.file) return; await fs.mkdir(path.dirname(this.file), { recursive: true }); const value = { works: [...this.works.values()], evidence: [...this.evidence.values()], handoffs: [...this.handoffs.values()], reviews: [...this.reviews.values()] }; const temp = `${this.file}.tmp`; await fs.writeFile(temp, JSON.stringify(value, null, 2), 'utf8'); await fs.rename(temp, this.file); }
