...
export class WorkflowStore {
  ...
  async load(): Promise<void> { if (!this.file) return; try { const raw = JSON.parse(await fs.readFile(this.file, 'utf8')) as Record<string, unknown>; for (const item of Array.isArray(raw.works) ? raw.works : []) { const w = item as WorkRecord; if (w?.id) this.works.set(w.id, w); } for (const item of Array.isArray(raw.evidence) ? raw.evidence : []) { const e = item as Evidence; if (e?.id) this.evidence.set(e.id, e); } for (const item of Array.isArray(raw.handoffs) ? raw.handoffs : []) { const h = item as Handoff; if (h?.id) this.handoffs.set(h.id, h); } for (const item of Array.isArray(raw.reviews) ? raw.reviews : []) { const r = item as Review; if (r?.id) this.reviews.set(r.id, r); } } catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return; const reason = error instanceof Error ? error.message : String(error); let preservedFile: string | undefined; try { preservedFile = `${this.file}.corrupt-${Date.now()}`; await fs.rename(this.file, preservedFile); } catch { /* preserve the original if the filesystem cannot rename it */ } this.recovery = { required: true, file: this.file, preservedFile, reason }; } }
  recoveryStatus(): { required: true; file: string; preservedFile?: string; reason: string } | null { return this.recovery ?? null; }
  private async save(): Promise<void> { if (!this.file) return; await fs.mkdir(path.dirname(this.file), { recursive: true, mode: 0o700 }); const lock = `${this.file}.lock`; let handle; for (let attempt = 0; attempt < 100; attempt++) { try { handle = await fs.open(lock, 'wx'); break; } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error; await new Promise((resolve) => setTimeout(resolve, 10)); } } if (!handle) throw new Error('Could not acquire workflow state lock'); try { try { const raw = JSON.parse(await fs.readFile(this.file, 'utf8')) as Record<string, unknown>; for (const item of Array.isArray(raw.works) ? raw.works : []) { const w = item as WorkRecord; if (w?.id && !this.works.has(w.id)) this.works.set(w.id, w); } for (const item of Array.isArray(raw.evidence) ? raw.evidence : []) { const e = item as Evidence; if (e?.id && !this.evidence.has(e.id)) this.evidence.set(e.id, e); } for (const item of Array.isArray(raw.handoffs) ? raw.handoffs : []) { const h = item as Handoff; if (h?.id && !this.handoffs.has(h.id)) this.handoffs.set(h.id, h); } for (const item of Array.isArray(raw.reviews) ? raw.reviews : []) { const r = item as Review; if (r?.id && !this.reviews.has(r.id)) this.reviews.set(r.id, r); } } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; } const value = { works: [...this.works.values()], evidence: [...this.evidence.values()], handoffs: [...this.handoffs.values()], reviews: [...this.reviews.values()] }; const temp = `${this.file}.${randomUUID()}.tmp`; await fs.writeFile(temp, JSON.stringify(value, null, 2), { encoding: 'utf8', mode: 0o600 }); await fs.rename(temp, this.file); if (process.platform !== 'win32') await fs.chmod(this.file, 0o600); } finally { await handle.close(); await fs.unlink(lock).catch(() => undefined); } }
