...
let ptyModulePromise: Promise<PtyModule> | undefined;
...
function ptyUnavailableMessage(error: unknown): string {
  const detail = error instanceof Error ? error.message : String(error);
  return `node-pty is optional and could not be loaded: ${detail}. Base Agent Interop features remain available; Freebuff CLI PTY mode requires a working node-pty install for this Node/platform combination.`;
}
...
export async function probePty(): Promise<PtyDiagnostics> {
  ...
  try {
    ...
    const result = await new Promise<PtyDiagnostics>((resolve) => { let output = ''; let settled = false; const finish = (value: PtyDiagnostics) => { if (settled) return; settled = true; clearTimeout(timer); try { term.kill(); } catch {} resolve(value); }; const timer = setTimeout(() => finish({ ok: false, node: process.version, platform: process.platform, nodePty: NODE_PTY_VERSION, error: 'PTY probe timed out' }), 1500); term.onData((data) => { output += data; }); term.onExit(({ exitCode }) => { finish(exitCode === 0 ? { ok: true, node: process.version, platform: process.platform, nodePty: NODE_PTY_VERSION } : { ok: false, node: process.version, platform: process.platform, nodePty: NODE_PTY_VERSION, error: `probe exited with code ${exitCode}; output ${output.slice(-200)}` }); }); });
    return result;
  } catch (error) { return { ok: false, node: process.version, platform: process.platform, nodePty: NODE_PTY_VERSION, error: ptyUnavailableMessage(error) }; }
}
...
export class CliPtyManager {
  private sessions = new Map<string, { term: IPty; cwd: string; startedAt: number; conversationId?: string; output: string; exited: boolean; exitCode?: number; events: ThreadProgressEvent[]; sequence: number; state: ThreadProgressEvent['state'] }>();
  ...
  async start(id: string, cwd: string, continueId?: string): Promise<CliSessionSnapshot> {
    ...
    let pty: PtyModule;
    try { pty = await loadPty(); } catch (error) { throw new Error(`FREEBUFF_PTY_UNAVAILABLE: ${ptyUnavailableMessage(error)}`); }
    let term: IPty;
    try { term = pty.spawn(file, args, { name: 'xterm-256color', cols: 160, rows: 48, cwd, ...(process.platform === 'win32' ? { useConpty: true } : {}), env: { ...process.env, TERM: 'xterm-256color' } }); } catch (error) { throw new Error(`FREEBUFF_PTY_START_FAILED: ${error instanceof Error ? error.message : String(error)}. Reinstall or rebuild the optional node-pty dependency for ${process.version} if Freebuff CLI PTY mode is required.`); }
    ...
    term.onData((data) => { state.output = (state.output + data).slice(-2_000_000); const visible = visibleTerminalText(data); if (/not authenticated|authentication failed|\bfatal\b/i.test(visible)) { state.state = 'failed'; this.progress(safeId, 'failed', visible.slice(-500), visible.slice(-500)); } else if (/\b(?:turn|task|session)\s+(?:completed|finished)\b|state\s*[:=]\s*idle/i.test(visible)) { state.state = 'completed'; this.progress(safeId, 'completed', visible.slice(-500)); } });
  ...
  async send(id: string, text: string, cwd = process.cwd(), continueId?: string): Promise<CliSessionSnapshot> { if (!text || text.length > 100_000) throw new Error('Message must be 1 to 100000 characters'); const safeId = assertSafeId(id); const previous = this.writeQueues.get(safeId) ?? Promise.resolve(); const result = previous.catch(() => undefined).then(() => this.writePrompt(safeId, text, cwd, continueId)); this.writeQueues.set(safeId, result.then(() => undefined, () => undefined)); return result; }
  ...
  stop(id: string): CliSessionSnapshot { const state = this.sessions.get(assertSafeId(id)); if (!state) throw new Error('FREEBUFF_CLI_SESSION_NOT_FOUND'); state.term.write('\x1b'); return this.snapshotState(assertSafeId(id)); }
