...
export class OpenCodeAdapter implements AgentAdapter {
  ...
  async send(nativeId: string, value: string, options?: { model?: string; variant?: string }): Promise<OperationReceipt> { await this.request('POST', `/session/${encodeURIComponent(nativeId)}/prompt_async`, { ...(options?.model ? { model: options.model } : {}), ...(options?.variant ? { agent: options.variant } : {}), parts: [{ type: 'text', text: value }] }); return { provider: this.id, nativeId, operation: 'send', accepted: true, status: 'queued', providerState: 'transport_accepted', detail: json({ delivery: 'transport_accepted', completion: 'not_observed' }) }; }
  async steer(nativeId: string, value: string, options?: { model?: string; variant?: string }): Promise<OperationReceipt> { return this.send(nativeId, value, options); }
