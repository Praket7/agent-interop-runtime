...
import type { AgentAdapter, AgentCapabilities, AgentEvent, AgentSession, OperationReceipt, ProviderId, AgentSendOptions } from './interop.js';
...
export class OpenCodeAdapter implements AgentAdapter {
  ...
  async setModel(_nativeId: string, _model: string): Promise<OperationReceipt> { throw new Error('OpenCode model mutation is not supported; pass model with agent_send'); }
...
export class ClaudeCodeAdapter extends NativeProtocolAdapter {
  ...
  protected initialize() { return this.call('initialize', { protocolVersion: 1, clientInfo: { name: 'agent-interop-runtime', version: '0.2.5' }, clientCapabilities: { fs: { readTextFile: true }, terminal: true } }); }
  async capabilities(): Promise<AgentCapabilities> { const base = await super.capabilities(); const ready = base.discovery.supported; return { ...base, model: cap(ready, 'ACP JSON RPC', this.protocol, ready ? 'Set with session/set_config_option using configId model or session/set_model when advertised' : base.discovery.reason), reasoning: cap(ready, 'ACP JSON RPC', this.protocol, ready ? 'Set with session/set_config_option using the advertised thought level option' : base.discovery.reason), limitations: [...base.limitations, 'Claude controls are accepted only when the ACP agent advertises the corresponding session configuration option'] }; }
  protected async discover() { try { const result = record(await this.call('session/list', {})); const sessions = result.sessions ?? result.data ?? result; return Array.isArray(sessions) ? sessions : []; } catch { return []; } }
  ...
  protected resume(nativeId: string) { return this.call('session/load', { sessionId: nativeId, cwd: this.session(nativeId)?.cwd ?? process.cwd(), mcpServers: [] }); }
  ...
  async send(nativeId: string, value: string, options?: AgentSendOptions): Promise<OperationReceipt> { if (options?.model) await this.setModel(nativeId, options.model.modelID); if (options?.reasoning) await this.setReasoning(nativeId, options.reasoning); if (options?.agent) await this.setConfig(nativeId, 'agent', options.agent); return super.send(nativeId, value); }
  private async setConfig(nativeId: string, configId: string, value: string): Promise<void> { await this.call('session/set_config_option', { sessionId: nativeId, configId, type: 'id', value }); }
  async setModel(nativeId: string, model: string): Promise<OperationReceipt> { try { await this.setConfig(nativeId, 'model', model); } catch { await this.call('session/set_model', { sessionId: nativeId, modelId: model }); } return { provider: this.id, nativeId, operation: 'set_model', accepted: true, status: 'completed', providerState: 'configuration_applied' }; }
