...
export class DesktopOrchestratorRuntime implements Runtime {
  ...
  async capabilities(): Promise<Capabilities> { if (this.caps) return this.caps; try { const base = await discoverDesktopUrl(); this.base = new URL(base); const projects = await this.request<{projects?: unknown[]}>('GET','/api/projects'); const endpoints = ['/api/projects','/api/thread/:id','/api/thread/:id/message','/api/thread/:id/stop','/api/thread/:id/resume','/api/thread/:id/agent','/api/thread/:id/effort','/api/thread/:id/attachment']; this.caps = { product:'desktop', signedIn:'unknown', orchestrator:true, readOnly:false, endpoints, notes:[`Live Desktop API responded with ${projects.projects?.length ?? 0} recent project roots. Model catalog is embedded in the app and is validated by the thread action response.`] }; } catch { this.caps = { product:'unknown', signedIn:'unknown', orchestrator:false, readOnly:true, endpoints:[], notes:['No Freebuff Desktop orchestrator responded on the discovered localhost URL.'] }; } return this.caps; }
  ...
  async getThread(id:string):Promise<ThreadDetail>{return redact(await this.request('GET',`/api/thread/${encodeURIComponent(assertSafeId(id))}`)) as ThreadDetail;}
  async getMessages(id:string):Promise<Json>{const t=await this.getThread(id); return redact(t.messages ?? []) as Json;}
...
export async function detectRuntime(): Promise<Runtime> { const r=new DesktopOrchestratorRuntime(await discoverDesktopUrl()); if((await r.capabilities()).orchestrator)return r; return new ReadOnlyRuntime(await discoverDesktopUrl()); }
