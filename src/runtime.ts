...
export class CliPtyRuntime implements Runtime {
  ...
  async sendMessage(id:string,text:string): Promise<Json> { if ((await this.capabilities()).readOnly) throw new Error('Freebuff CLI PTY control is unavailable'); return redact(await this.manager.send(id,text,this.root)) as Json; }
