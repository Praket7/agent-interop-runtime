...
export function normalizeProgressEvent(value: unknown, eventName?: string): Omit<ThreadProgressEvent, 'sequence'> | null {
  ...
  const out: Omit<ThreadProgressEvent, 'sequence'> = { threadId, timestamp, kind, state:text(input.state) ?? text(input.turnState) ?? text(input.status), tool:text(input.tool) ?? text(input.toolName), command:text(input.command), text:text(input.text) ?? text(input.content) ?? text(input.message), files, error:text(input.error) };
...
export class ProgressStore {
  ...
  read(threadId: string, afterSequence = 0, limit = 50): ThreadProgressSnapshot { const now = Date.now(); const entries = (this.byThread.get(threadId) ?? []).filter(x => now - Date.parse(x.event.timestamp) <= TTL_MS); this.byThread.set(threadId, entries); const events = entries.map(x => x.event).filter(x => x.sequence > afterSequence).slice(0, Math.max(1, Math.min(limit, 100))); const all = entries.map(x => x.event); const latest = all.at(-1); const currentState = [...all].reverse().find(x => x.state)?.state; const activeTool = [...all].reverse().find(x => x.tool && (x.kind === 'tool_start' || x.kind === 'tool_output'))?.tool; const filesChanged = [...new Set(all.flatMap(x => x.files ?? []))].slice(-100); return { threadId, currentState, events, nextSequence: latest?.sequence, connected: this.connected, stale: !this.connected || (this.lastConnectionAt > 0 && Date.now() - this.lastConnectionAt > STALE_MS), latestEventAt: latest?.timestamp, activeTool, filesChanged }; }
