...
export function normalizeProgressEvent(value: unknown, eventName?: string): Omit<ThreadProgressEvent, 'sequence'> | null {
  ...
  const phase = rawType.includes('reasoning') || rawType.includes('plan') ? 'planning' : rawType.includes('read') ? 'reading_files' : rawType.includes('test') ? 'running_tests' : rawType.includes('edit') || rawType.includes('write') ? 'editing_files' : rawType.includes('review') ? 'reviewing_changes' : rawType.includes('input') || rawType.includes('approval') ? 'waiting_for_input' : kind === 'completed' ? 'completed' : kind === 'failed' ? 'failed' : tool === 'shell' || command ? 'running_tests' : undefined;
  ...
  const out: Omit<ThreadProgressEvent, 'sequence'> = sanitizeFreebuff({ threadId, timestamp, kind, phase, state:text(input.state) ?? text(input.turnState) ?? text(input.status), tool, command, text: rawType.includes('reasoning') ? undefined : text(input.text) ?? text(input.content) ?? text(input.message), files, error:text(input.error) }) as Omit<ThreadProgressEvent, 'sequence'>;
...
export class DesktopEventClient {
  ...
  constructor(private readonly base: () => URL, private readonly launchId: () => string | undefined, private readonly store: ProgressStore, private readonly refresh?: () => Promise<boolean>) {}
