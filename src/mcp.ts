...
export function createServer(runtime: Runtime, includeWrites = true): McpServer { const s=new McpServer({name:'agent-interop-runtime',version:VERSION}); const interop=createInteropRegistry(runtime); const workflow=new WorkflowStore(process.env.INTEROP_STATE_FILE ?? path.join(os.homedir(), '.agent-interop-runtime', 'state.json')); const ready=workflow.load();
  ...
  write('permission_respond','Respond to a provider permission request when supported.',{provider:z.enum(['freebuff','opencode','codex','claude-code']),nativeId:z.string(),requestId:z.string(),decision:z.string()},(a)=>interop.permission(a.provider as ProviderId,a.nativeId,a.requestId,a.decision));
  write('session_set_model','Change the model for an exact native session when supported.',{provider:z.enum(['freebuff','opencode','codex','claude-code']),nativeId:z.string(),model:z.string()},(a)=>interop.model(a.provider as ProviderId,a.nativeId,a.model));
  write('session_set_reasoning','Change reasoning effort for an exact native session when supported.',{provider:z.enum(['freebuff','opencode','codex','claude-code']),nativeId:z.string(),effort:z.string()},(a)=>interop.reasoning(a.provider as ProviderId,a.nativeId,a.effort));
